...
`BrowserConfig` 分开控制来源 HTTP 预取并发（默认全局 8、同域 2）、正文并发（默认全局 3、同域 1）、profile、浏览器 channel、导航超时和默认等待；`BudgetConfig` 控制 600 秒预算、Agent token 契约、context 每来源候选数、日报每来源上限和正文硬上限。
