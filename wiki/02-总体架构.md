...
负责配置解析、浏览器生命周期、来源状态、ID、revision、锁、文件写入、Schema、跨字段验证、本地 HTML/PDF 渲染、Notion 幂等和评估调度。这里的目标是确定性与可测试性。
...
```mermaid
flowchart TB
    CLI[cli.py] --> CFG[config.py / runtime.py]
    CLI --> VER[verification.py]
    CLI --> WF[workflow.py]
    WF --> COL[collector.py]
    COL --> PRE[prefetch.py]
    COL --> ADP[adapters.py]
    WF --> CTX[context.py / semantics.py]
    WF --> CNT[content.py]
    WF --> REP[reports.py]
    REP --> CMP[reporting.py]
    REP --> LOC[local_output.py]
    REP --> STA[state.py]
    WF --> NOT[notion.py]
    CLI --> NOT
    COL --> STO[storage.py]
...
| 模块 | 允许做什么 | 不应做什么 |
| --- | --- | --- |
| `cli.py` | 参数解析、命令分发、加载 Hermes `.env` | 实现核心采集、验证前端或报告规则 |
| `workflow.py` | 状态迁移、锁、阶段组合、失败恢复、评估调度 | 解析站点 DOM、生成 TL;DR |
| `config.py` / `runtime.py` | 配置、路径优先级、唯一数据根、动态栏目页 | 抓网页、保存报告 |
| `collector.py` / `prefetch.py` | 有界 HTTP 预取、Edge 回退、挑战分类、合并页面、写 index | 并发操作同一持久化 profile |
| `verification.py` | 本地队列 UI、标签监听、验证后采集与 adoption | 绕过 CAPTCHA、付费墙或限流 |
| `adapters.py` | 来源协议和过滤、输出统一 `ArticleItem` | 管理 run 状态或 Notion |
| `content.py` | 精选正文并发、内容状态、正文文件 | 决定哪些新闻重要 |
| `context.py` / `semantics.py` | 压缩候选、评估门控缓存、历史隔离、批次和计划 | 生成新翻译、TL;DR 或研判 |
| `reporting.py` | 编译草稿、注入权威引用、校验跨字段关系 | 替模型补写语义 brief |
| `reports.py` | revision、保存、Markdown 与投影编排 | 直接抓网页 |
| `local_output.py` | 安全 HTML、日报索引、Edge/ReportLab PDF、评估后刷新 | 修改报告事实或调用模型 |
...
唯一例外是本地交付后评估调度：`workflow.schedule_independent_evaluation()` 通过 `subprocess.run()` 调用 Hermes CLI 创建独立 Cron 任务，这是刻意隔离模型上下文，而不是复用当前生成 Agent。
...
reports/*-rN.md                  ← 不可变：Markdown 事实源
reports/*-rN.html/.pdf           ← 可重建：本地阅读投影
reports/index.html               ← 可重建：本地日报索引
...
生成 Agent 需要综合上下文并快速交付；评估 Agent 只应看到不可变结果。交付流程因此是：

```text
生成 → Python 验证 → 保存 JSON/Markdown → HTML/PDF → 返回成功
                                      ├→ 可选 Notion
                                      └→ 异步创建独立评估任务
```
...
`local_output.py` 与 Notion publisher 都只消费已经保存并通过验证的 report JSON。前者始终生成本地 HTML/PDF 和索引，后者仅在显式 `--publish` 时运行；二者都不直接读取网页、不访问 Hermes context，也不在投影阶段修正语义。评估完成时只重新渲染 HTML/PDF 的评估区，不改写不可变报告或内容哈希。未来增加邮件或静态站点时，只需新增投影层，不改采集和报告契约。
